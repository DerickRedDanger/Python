...
class FunctionLogger:
    ...
    def log_function(self, tag=None, error_return=None,
        ...
        def decorator(func):
            ...
            def wrapper(*args, **kwargs):
                ...
                # Also, store the *actual* arg/kwarg snapshots in the registry if their modes are not 'str' or 'None'
                ...
                finalized = self.tracker_exit(log_id, result, error, warnings_list, result_snap_mode=res_mode)
    ...
    def tracker_exit(self, log_id, result, error, warnings_list, result_snap_mode=None):
        """
        ...
        end_time = time.time()
        frame_log["timestamp_end"] = datetime.fromtimestamp(end_time).strftime(
    "%Y-%m-%d %H:%M:%S.%f")[:-3]
        frame_log["duration"] = round(end_time - frame["start_time"], 4)
        frame_log["result type"] = arg_type(result)
        frame_log["result"] = snapshot(result, "str")
        frame_log["warnings"] = warnings_list or[]
